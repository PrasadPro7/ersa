...
class ERSAProgramGeneratorGUI:
    ...
    def create_metadata_tab(self):
        tab = ttk.Frame(self.notebook, padding='20')
        self.notebook.add(tab, text='ðŸ“ Metadata & IDs')
        current_user = getpass.getuser()
        self.meta_programid_start = tk.IntVar(value=2175)
        self.meta_libraryid_start = tk.IntVar(value=455)
        self.meta_version = tk.IntVar(value=3)
        self.meta_setnumber_start = tk.IntVar(value=2)
        self.meta_creationuser = tk.StringVar(value=current_user)
        self.meta_changeuser = tk.StringVar(value=current_user)
        self.meta_default_prefix = tk.StringVar(value="SolderingPrograms")
        self.meta_default_notes = tk.StringVar(value="Template")
        row = 0
        ttk.Label(tab, text="Starting Program ID (auto-increment):").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_programid_start, width=12).grid(row=row-1, column=1)
        ttk.Label(tab, text="Starting Library ID:").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_libraryid_start, width=12).grid(row=row-1, column=1)
        ttk.Label(tab, text="Default Program Version:").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_version, width=5).grid(row=row-1, column=1)
        ttk.Label(tab, text="Starting Setnumber (auto-increment):").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_setnumber_start, width=6).grid(row=row-1, column=1)
        ttk.Label(tab, text="Default Name Prefix:").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_default_prefix, width=15).grid(row=row-1, column=1)
        ttk.Label(tab, text="Creation User:").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_creationuser, width=15).grid(row=row-1, column=1)
        ttk.Label(tab, text="Change User:").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_changeuser, width=15).grid(row=row-1, column=1)
        ttk.Label(tab, text="Default Notes:").grid(row=row, column=0, sticky=tk.W); row+=1
        ttk.Entry(tab, textvariable=self.meta_default_notes, width=35).grid(row=row-1, column=1)
        ttk.Label(tab, text="Creation/Change date will be set at time of generation.", foreground="gray").grid(row=row, column=0, columnspan=2, sticky=tk.W)

        # Note: Do not iterate self.df here. Metadata values are applied per-program during generation.
        # This UI tab only lets user configure starting IDs / defaults.
    #==================== END META DATA =====================
    ...
    def update_program_metadata(self, root, program_name, row_index):
        ...
        try:
            ...
            libraryid_base = self.meta_libraryid_start.get()
            setnumber_base = self.meta_setnumber_start.get()
            version = self.meta_version.get()
            creationuser = self.meta_creationuser.get()
            changeuser = self.meta_changeuser.get()
            notes = self.meta_default_notes.get()
            prefix = self.meta_default_prefix.get()

            prog_id = programid_base + row_index
            lib_id = libraryid_base
            setnum = setnumber_base 
            prog_name = f"{prefix}{program_name}"
            now = datetime.now().isoformat()

            # Best-effort update of common nodes
            soldering_programs = root.find('SolderingPrograms')
            if soldering_programs is not None:
                def set_text(elem_name, value):
                    e = soldering_programs.find(elem_name)
                    if e is None:
                        return
                    e.text = str(value)

                set_text('programid', prog_id)
                set_text('libraryid', lib_id)
                set_text('version', version)
                set_text('creationuser', creationuser)
                set_text('changeuser', changeuser)
                set_text('creationdate', now)
                set_text('changedate', now)
                set_text('name', prog_name)

            # ProgramHistory (if present)
            prog_history = root.find('ProgramHistory')
            if prog_history is not None:
                sn = prog_history.find('setnumber')
                if sn is not None:
                    sn.text = str(setnum)
        except Exception:
            # swallow to avoid stopping generation; detailed trace logged by caller
            raise
